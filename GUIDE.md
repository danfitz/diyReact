...
`useState` also needs to return a `setState` function. This function basically

1. Receives an action that you enqueue, and then
...
DiyReact.useState = (initial) => {
  ...
  const setState = (action) => {
    ...
    nextUnitOfWork = wipRoot;
    deletions = []; // we reset deletions too, since it's a new render
  };
...
One last thing to add: when `setState` is in fact invoked and the render phase gets retriggered, we need to handle the enqueued actions.
